...
#include "ControlsForTrampoline.as";
#include "Hitters.as";
...
void onInit(CBlob@ this)
{
	...
	this.addCommandID("freeze_angle_at client");
	this.addCommandID("unfreeze_tramp client");
	this.addCommandID("unfold");
	this.addCommandID("unfold client");
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1, Vec2f point2)
{
	if (blob is null || blob.getShape().isStatic())
	{
		if (solid && this.hasTag("folded") && !this.isAttached())
			this.SendCommand(this.getCommandID("unfold client"));
...
{
	if (cmd == this.getCommandID("freeze_angle_at") && isServer())
	{
		...
		this.set_f32("old_angle", angle);
		this.Sync("old_angle", true);

		this.setAngleDegrees(angle);
		ShowMeYourFeet(this, angle);
		
		this.SendCommand(this.getCommandID("freeze_angle_at client"));
	}
	else if (cmd == this.getCommandID("unfreeze_tramp") && isServer())
	{
		CPlayer@ p = getNet().getActiveCommandPlayer();
		if (p is null) return;
		...
		this.SendCommand(this.getCommandID("unfreeze_tramp client"));
	}
	else if (cmd == this.getCommandID("unfold") && isServer())
	{
		CPlayer@ p = getNet().getActiveCommandPlayer();
		if (p is null) return;
	...
	if (cmd == this.getCommandID("freeze_angle_at client") && isClient())
	{
		f32 angle = this.get_f32("old_angle");

		this.setAngleDegrees(angle);
		ShowMeYourFeet(this, angle);
	}
	else if (cmd == this.getCommandID("unfreeze_tramp client") && isClient())
	{
		Fold(this);
		RemoveFeet(this);
	}
	else if (cmd == this.getCommandID("unfold client") && isClient())
	{
		Unfold(this);
	}
}
